...
## 🧪 FAILING TESTS (40 tests, 11 files)

### TD-003: SSO Route Tests Expect Unimplemented Endpoints
...
**Problem:**
19 tests in `apps/api/tests/unit/routes/sso-test.test.ts` are failing. The tests were added in commits `e5f98bc`, `a6ddbeb`, `1caedc2` but the corresponding route implementations either don't exist or have mismatched behavior.

**Failing Tests:**
- `should return 404 when provider not configured`
- `Google provider > should return error when clientId format invalid`
- `Google provider > should return success when credentials format valid`
- `Plex provider > should return info message about PIN auth`
- `SAML provider > should return success when metadata URL returns valid XML`
- `SAML provider > should return error when metadata URL unreachable`
- `SAML provider > should validate manual config when no metadata URL`
- `LDAP provider > should return success when LDAP bind succeeds`
- `LDAP provider > should return error when LDAP bind fails`

**Root Cause:**
Tests expect specific response codes/messages that don't match implementation. For example, tests expect 200 with `success: false` for bind failures, but route returns 500.

**Files Affected:**
- `apps/api/tests/unit/routes/sso-test.test.ts`
- `apps/api/src/routes/sso.ts` (implementation)

---

### TD-004: slskd Integration Tests Have Mock/Implementation Mismatch
...
**Problem:**
~15 tests across multiple slskd test files are failing due to mock configuration issues or implementation changes that weren't reflected in tests.

**Failing Test Files:**
- `tests/services/slskd-subscription-processor.test.ts` (9 failures)
  - Per-File Download Model tests
  - Search Retry Logic tests
  - Rate-Limited Queue Integration tests
- `tests/api/slskd-downloads.test.ts`
- `tests/api/slskd.test.ts`
- `tests/integration/slskd-rate-limiting.test.ts`
- `tests/security/slskd-security.test.ts`
- `tests/jobs/slskd-operations-worker.test.ts`
- `tests/lib/settings.test.ts`

**Root Cause:**
The slskd integration was added rapidly and tests were written before implementation was finalized. Mocks don't match actual service signatures or return values.

**Files Affected:**
- All files in `apps/api/tests/**/slskd*.test.ts`
- `apps/api/src/services/slskd/` (implementation)

---

### TD-005: Miscellaneous Test Failures
...
**Problem:**
Additional test failures in:
- `tests/api/public-playlist.test.ts`
- `tests/controllers/subscriptions.controller.test.ts`
- `tests/services/jellyfin.test.ts`
